...
describe("Clip Player (Scheduler adapter)", () => {
  ...
  describe("Real clips", () => {
    ...
    it("adds clip content to scheduler when a second clip is added", () => {
      ...
      expect(SchedulerInstance.events["content1id"].start).toBeCloseTo(-0.16);
    ...
    it("when added, clip content is cut off if clip ends first", () => {
      mount(
        <ClipPlayer
          clips={[
            {
    ...
    it("updates scheduler event duration for content if content start changed", () => {
      ...
      clipPlayer.setProps({ clips: newClipMocks });
      expect(SchedulerInstance.events["content1id"].start).toBeCloseTo(-1.16);
    });
